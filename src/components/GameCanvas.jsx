...
export default function GameCanvas({
  ...
  // Use refs to store mutable state for the tick loop
  ...
  // Sync selected block prop to tick loop state
  useEffect(() => {
    stateRef.current.selectedBlockIndex = selectedBlock;
  }, [selectedBlock]);
  ...
  useEffect(() => {
    ...
    function setBlock(x, y, z, type) {
      ...
      if (type) {
        state.world.set(key, type);
      } else {
        state.world.delete(key);
      }
    }

    function getBlock(x, y, z) {
      return state.world.get(worldKey(x, y, z)) || null;
    }
    ...
    function buildWorld() {
      ...
      const startHeight = findGround(state.player.x, state.player.z);
      state.player.y = startHeight + 3;
    }
    ...
    function updatePlayer(dt) {
      ...
      if (positionRef.current) {
        positionRef.current.textContent = `${player.x.toFixed(1)}, ${player.y.toFixed(1)}, ${player.z.toFixed(1)}`;
      }
    }

    function projectPoint(point) {
      const cx = point.x - state.player.x;
      const cy = point.y - (state.player.y + EYE_HEIGHT);
      const cz = point.z - state.player.z;
      ...
      const sinPitch = Math.sin(-state.player.pitch);
        ...
        for (let z = pz - RENDER_DISTANCE; z <= pz + RENDER_DISTANCE; z += 1) {
          for (let y = 0; y <= WORLD_HEIGHT + 4; y += 1) {
            ...
            for (const face of faceDefinitions) {
              ...
              const toCamera = {
                x: state.player.x - center.x,
                y: state.player.y + EYE_HEIGHT - center.y,
                z: state.player.z - center.z,
              };
              const facing =
              ...
              faces.push({
                ...
                outline: state.highlighted &&
                  state.highlighted.block.x === x &&
                  state.highlighted.block.y === y &&
                  state.highlighted.block.z === z &&
                  state.highlighted.face === face.name,
              });
            }
          }
        }
    ...
    function raycast(maxDistance = 8) {
      ...
      for (let step = 0; step < maxDistance * 20; step += 1) {
        ...
        const x = state.player.x + direction.x * distance;
        const y = state.player.y + EYE_HEIGHT + direction.y * distance;
        const z = state.player.z + direction.z * distance;
    ...
    function breakBlock() {
      state.highlighted = raycast();
      if (!state.highlighted) return;
      const { x, y, z } = state.highlighted.block;
      setBlock(x, y, z, null);
    }

    function blockIntersectsPlayer(x, y, z) {
      const playerMinX = state.player.x - PLAYER_RADIUS;
      const playerMaxX = state.player.x + PLAYER_RADIUS;
      const playerMinY = state.player.y;
      const playerMaxY = state.player.y + PLAYER_HEIGHT;
      const playerMinZ = state.player.z - PLAYER_RADIUS;
      const playerMaxZ = state.player.z + PLAYER_RADIUS;
    ...
    function placeBlock() {
      state.highlighted = raycast();
      if (!state.highlighted || !state.highlighted.previous) return;
      const { x, y, z } = state.highlighted.previous;

      if (y < 0 || y > WORLD_HEIGHT + 5) return;
      if (blockIntersectsPlayer(x, y, z)) return;

      setBlock(x, y, z, blockTypes[state.selectedBlockIndex].id);
    }
    ...
    function handleMouseMove(e) {
      ...
      state.player.pitch = clamp(state.player.pitch, -1.3, 1.3);
    }
  ...
  }, [blockTypes, onSelectBlock, positionRef]);
